...
beijing_long |>
  ...
  scale_y_continuous(trans = "log10") +
  facet_wrap(~pollutant, scales = "free") +
  scale_x_discrete(labels = c("2AM", "8AM", "2PM", "8PM"), breaks = c(3, 9, 15, 21))
```
...
-   Use either `geom_segment` or `geom_path` to draw a small figure around the location of each station, where each point in the path or segment drawn corresponds to a wind direction, and the length of it is proportional to the mean concentration, or log of the mean concentration. Some starter code is below.
...
**f. (Optional)** Please share any feedback about this assignment. Some things to consider: 
